...
class Version_1_20_3(Version):
    ...
    def send_join_game(self):
        self.protocol.send_packet("join_game",
                                  ...
                                  self.protocol.buff_type.pack_varint(0),  # Max players
                                  self.protocol.buff_type.pack_varint(7),  # View distance
                                  self.protocol.buff_type.pack_varint(0),  # Simulation distance
                                  self.protocol.buff_type.pack("???", False, True, False),  # Flat, debug, limited crafting
                                  self.protocol.buff_type.pack_string(str(self.current_world.dimension)),  # Current dimension
                                  self.protocol.buff_type.pack_string("rtgame:waiting"),  # Current world name
                                  self.protocol.buff_type.pack("qBb??", 0, 1, 1, False, False),  # Hashed seed, gamemode, prev gamemode
                                  self.protocol.buff_type.pack("?", False),  # Optional death location
